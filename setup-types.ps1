## VOLT - Etape 12: Types + Maps
Write-Host "`n  VOLT - Installing types & maps...`n" -ForegroundColor Cyan
                ...
                OP_CHECK_TYPE => {
                    ...
                    if !ok {
                        let actual = if val.is_nil() { "nil" }
                            ...
                            else if val.is_obj() { match self.heap.get(val.as_obj()) { Obj::Str(_) => "str", Obj::Array(_) => "array", Obj::Map(_) => "map", Obj::Struct { .. } => "struct", Obj::Closure { .. } => "fn" } }
                            else { "unknown" };
...
// → 1
'@
[System.IO.File]::WriteAllText("$PWD/tests/programs/type_test.volt", $f10)
Write-Host "  [OK] tests/programs/type_test.volt" -ForegroundColor Green


Write-Host "`n  Types & maps installed!" -ForegroundColor Cyan
Write-Host "  Test: cargo run -- run tests/programs/type_test.volt"
